    ...
    return <>
        { displayNameBody.selectedFormat.fields.map((field, index) => {
            const props = {
                ...
                onChange : (e) => { onHandleFieldChange(e, index); },
            ...
            if (field.type == types.text) {
                return (
                    <Text key={displayNameBody.selectedFormat.name + index} {...props} />
                );
            } else if (field.type == types.number) {
                return (
                    <Number key={displayNameBody.selectedFormat.name + index} {...props} />
                );
            } else if (field.type == types.select) {
                return (
                    <Select key={displayNameBody.selectedFormat.name + index} {...props} />
                );
                ...
                return (
                    <Count key={displayNameBody.selectedFormat.name + index} {...props} />
                );
            }
        })}
    </>
